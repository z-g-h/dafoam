    ...
    KSP ksp)
{
    ...
    */
    ...
    PetscInt surfaceState_indices[nLocalFaces];
    forAll(mesh_.faces(), faceI)
    {
        PetscInt globalIdx = daIndex_.getGlobalAdjointStateIndex("phi", faceI);
        surfaceState_indices[faceI] = globalIdx;
    }
    ISCreateGeneral(PETSC_COMM_WORLD, nLocalFaces, surfaceState_indices,  PETSC_COPY_VALUES, &is_phi);
    ISSort(is_phi);
